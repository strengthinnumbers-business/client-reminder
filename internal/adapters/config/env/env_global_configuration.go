...
func (c *GlobalConfiguration) GetEmailBodyTemplate(sequenceIndex int, style string) (string, string, error) {
	...
	c.logger.DemoBelow("loading email template configuration", "sequence_index", sequenceIndex, "style", style)
	...
	if c.templatePath != "" {
		c.logger.DemoBelow("loading email body template from file", "path", c.templatePath)
		bytes, err := os.ReadFile(c.templatePath)
		if err != nil {
			return "", "", fmt.Errorf("read template file: %w", err)
		}
		c.logger.DemoAbove("loaded email body template from file", "path", c.templatePath, "body_bytes", len(bytes))
	...
	c.logger.DemoAbove("loaded email body template from environment", "body_bytes", len(tpl))
