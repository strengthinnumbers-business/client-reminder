...
func (s *ReminderService) Run(ctx context.Context) (RunResult, error) {
	...
	for _, client := range clients {
		...
		match, ok, err := schedule.MatchAt(now, s.holidayChecker)
		if err != nil {
			result.Failures++
			continue
		}
		...
		log.Printf(
			...
			client.ID,
			match.Period.ID,
			match.SequenceDayOffset,
			match.SequenceIndex,
		)

		verdict, err := s.completionDecider.IsCompleted(client, match.Period)
		if err != nil {
			result.Failures++
			continue
		}
		...
		if verdict == entities.CompletionUndecided || verdict == entities.CompletionIncomplete {
			...
			result.Sent++
			if err := s.completionDecider.ResetCompletionVerdict(client, match.Period); err != nil {
				result.Failures++
			}
		}
